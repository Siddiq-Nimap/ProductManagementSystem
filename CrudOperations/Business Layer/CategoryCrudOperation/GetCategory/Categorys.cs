...
namespace CrudOperations.Business_Layer.CategoryCrudOperation
{
    public class Categorys : ICategory
    {
        ...
        public async Task<List<ReportCLass>> GetReportAsync(int id)
        {
            var param = new SqlParameter("@LoginId", id);

            var data = await productdb.Database.SqlQuery<ReportCLass>("execute spReport @LoginId", param).ToListAsync();
